...
public:
    ...
    QMCrgUnit(vector <vec> positions,
              vector <vec> norms,
              vector <vec> planes,
    ...
    QMCrgUnit(const unsigned int & id,
              CrgUnitType * type,
              const unsigned int & molId);
    ...
    double getOccupationProbability(          ) {
        return _occupation_probability;
    }
    void  setOccupationProbability(double prob) {
        _occupation_probability = prob;
    }

    void setInDatabase(bool indb) { _in_database = indb; }
    bool getInDatabase(         ) { return _in_database; }

    double getTotalEnergy(const char *match="energy_*",
                          const char *nomatch="") {
        //return getDouble("energy_intra")+getDouble("energy_coulomb");
        double e = 0;

        map<string, double>::iterator iter;
        for(iter = DoubleValues().begin();
            iter!= DoubleValues().end();
            ++iter) {
...
inline QMCrgUnit::QMCrgUnit(vector <vec> positions,
                            vector <vec> norms,
                            vector <vec> planes,
                            const unsigned int & id,
                            CrgUnitType * type,
                            const unsigned int & molId) :
                  CrgUnit(positions, norms, planes, id, type, molId),
                  _occupation_probability(0.) { }


inline QMCrgUnit::QMCrgUnit(const unsigned int & id,
                            CrgUnitType * type,
                            const unsigned int & molId) :
                  CrgUnit(id, type, molId),
                  _occupation_probability(0),
                  _in_database(false) { }
